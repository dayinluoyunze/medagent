...
当前实现适合作为求职作品中的 AI 应用项目展示，也适合作为后续扩展评测、引用溯源和安全策略的基础版本。
...
每次问答会将基础运行指标写入 `logs/chat_metrics.jsonl`，便于后续做简单分析或面试演示。
...
> 独立开发医药知识问答 Agent，基于 Streamlit、LangChain 和 FAISS 实现多模型接入、知识库检索增强、长期记忆摘要与本地回退检索，支持多种结构化与非结构化知识源接入，用于提升垂直场景下问答系统的可用性与可信度。

## 面试可讲的重点

- 为什么医药问答比普通聊天更需要 RAG 和来源约束
- 为什么要设计向量检索失败后的关键词回退
- 多 Provider 接入时如何统一配置与调用方式
- 长期记忆摘要如何控制上下文长度和成本
- 这个项目下一步如何通过评测集证明效果
